...
http请求报文：请求行+头部字段+<CR><LF>

请求报文可能很长，一次不能读完，需用从状态机进行判定状态转移；

*******---***

主状态机<请求行，头部字段>

从状态机(行的读取状态）<完整的行，行出错，不完整的行>***行指的是请求行+头部字段>

服务器处理结果<请求不完善、需要继续读取客户端数据···>

LINE_STATUS parse_line()------用于解析一行内容，返回读取的行是否完整或者出错；

HTTP_CODE parse_requestline()----进行请求行判定，返回是否出错，如果请求行正确则将主状态机转移为分析头部字段；

HTTP_CODE parse_headers()----分析头部字段；

HTTP_CODE parse_content()------分析HTTP请求入口：
...
（1）创建监听socket，并accept（）；

（2）进入while(1)循环监听：

a.读取数据recv；

b.调入parse_content(),依据返回结果进行动作判定，如果收到的状态机为“未读完”状态，那么继续循环读取，然后进入状态机函数入口；读取完成并分析完成之后，进行给客户端发送数据。

***源码部分***

（1) main()中监听到新的客户端，此时新建connfd唯一标记客户端，并初始化http中的用户user[connfd]；在http中向epollfd中注册该connfd的读事件，并初始化有限状态机状态。在main中监听到有读事件发生时，调用http中的read_once，此时分为LT和ET两种读取模式；

(2)当read_once（）为true时，将事件放入请求队列，请求队列进行调用http中process(),首先调用process_read();

（3）通过while进行有限状态机转换，在创建初始化时，状态为分析请求行，然后分析line_header、line_context，进行状态转移，解析HTTP报文，解析完报文后进行do_request（CGI检测和数据库操作），依据GET或者POST进行处理；

（4）在do_request处理完成后，根据do_request状态在process中调用process_write向m_write_buf中写入响应报文，然后注册epollout事件，主线程检测写事件调用write发送给浏览器

（5）数据库连接需先创建数据库连接池，因为连接数据库也耗内存时间；

​	数据库连接池创建：懒汉模式创建数据库连接池；
...
{
    //m_read_idx表示缓冲区m_read_buf中数据的最后一个字节的下一个位置
    ...
    //m_read_buf表示读取缓冲区，加上上一次读取的，进行将recv接收的数据读取至m_read_buf，READ_BUFFER_SIZE - m_read_idx表示还能接收的最大字节数；
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        //同步线程登录校验
        //第一次输入IP:端口号，返回静态界面；
        用户在静态界面上进行不同的动作，根据HTML中的action确定POST动作；
        在*(p+1)中进行判定
        if (*(p + 1) == '3')
        {
            //如果是注册，先检测数据库中是否有重名的
